...
import CoinbaseProAuthentication
...
def stopMenu(update):
    update.message.reply_text('Ferma:', reply_markup=stop_keyboard())

def stop_keyboard():
  keyboard = [[InlineKeyboardButton('Ferma', callback_data='exit')]]
  return InlineKeyboardMarkup(keyboard)
...
updater.dispatcher.add_handler(CallbackQueryHandler(stop, pattern='exit'))
...
def currency(update, context):
        ...
        while esci:
            try:
                msx = "["+CoinbaseProAuthentication.getTime()+"]" + CoinbaseProAuthentication.algo()
                pathlib.Path("AlgoReport.txt").write_text(msx)
                message = CoinbaseProAuthentication.riassunto()
